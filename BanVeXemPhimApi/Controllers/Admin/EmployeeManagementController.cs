...
namespace BanVeXemPhimApi.Controllers.Admin
{
    ...
    public class EmployeeManagementController : BaseApiController<EmployeeManagementController>
    {
        ...
        /// <returns></returns>
        [HttpPut("Update")]
        public IActionResult Update(int id, EmployeeStoreRequest request)
        {
            try
            {
                var res = _employeeManagementService.Update(id, request);
                return Ok(new MessageData { Data = res });
            }
            catch (Exception ex)
            {
        ...
        public IActionResult Delete(int userId)
        {
            try
            {
                var res = _employeeManagementService.Delete(userId, UserId);
                return Ok(new MessageData { Data = res });
            }
            catch (Exception ex)
            {
