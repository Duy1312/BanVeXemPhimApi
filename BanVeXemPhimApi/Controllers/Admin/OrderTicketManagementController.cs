...
namespace BanVeXemPhimApi.Controllers.Admin
{
    ...
    public class OrderTicketManagementController : BaseApiController<OrderTicketManagementController>
    {
        ...
        /// <returns></returns>
        [HttpGet("Get")]
        public IActionResult Get(int limit, int page, string? email, string? name, string? numberPhone)
        {
            try
            {
                var res = _orderTicketManagementService.GetModels(limit, page, email, name, numberPhone);
