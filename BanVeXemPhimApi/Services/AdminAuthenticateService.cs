...
namespace BanVeXemPhimApi.Services
{
    public class AdminAuthenticateService
    {
        ...
        public object ChangePassword(int adminId, ChangePasswordRequest request)
        {
            try
            {
                ...
                var admin = _adminRepository.FindOrFail(adminId);
                if (Untill.CreateMD5(request.OldPassword) != admin.Password)
                {
                    throw new Exception("Mật khẩu cũ không chính xác!");
                }
                admin.Password = Untill.CreateMD5(request.NewPassword);
                _adminRepository.UpdateByEntity(admin);
