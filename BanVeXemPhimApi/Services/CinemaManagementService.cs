...
namespace BanVeXemPhimApi.Services
{
    public class CinemaManagementService
    {
        ...
        /// <returns></returns>
        public object GetCinemas(int limit, int page, string? name, int? status)
        {
            try
            {
        ...
        public object Update(int cinemaId, CinemaUpdateRequest request)
        {
            try
            {
                ...
                cinemaUpdate.Status = request.Status;
        ...
        public bool Delete(int cinemaId)
        {
            try
            {
                ...
                var scheduleList = _scheduleRepository.FindByCondition(row => row.CinemaId == cinemaId);
                if(scheduleList.Count() > 0)
                {
                    throw new ValidateError("Không thể xoá rạp, vì rạp đã có lịch chiếu");
                }
