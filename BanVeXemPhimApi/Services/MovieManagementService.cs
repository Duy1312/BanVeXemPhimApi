...
namespace BanVeXemPhimApi.Services
{
    public class MovieManagementService
    {
        ...
        /// <returns></returns>
        public object GetMovies(int limit, int page, string? name, string? author)
        {
            try
            {
                ...
                {
                    query = query.Where(row => row.Author.ToLower().Contains(author.ToLower()));
                }
        ...
        public object Update(int movieId, MovieUpdateRequest request)
        {
            try
            {
                ...
                movieUpdate.Price = request.Price;
        ...
        public bool Delete(int movieId)
        {
            try
            {
                ...
                var scheduleList = _scheduleRepository.FindByCondition(row => row.MovieId == movieId).Count();
                if (scheduleList > 0)
                {
                    throw new ValidateError("Phim đã được thêm vào lịch chiếu, không thể xóa!");
                }
