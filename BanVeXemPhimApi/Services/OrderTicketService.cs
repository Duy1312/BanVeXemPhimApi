    ...
    public class OrderTicketService
    {
        ...
        public object CompleteBookTicket(int userId, BookTicketRequest bookTicketRequest)
        {
            try
            {
                ...
                orderTicket.TicketReceiptCode = DateTime.Now.ToString("MMddyyyyHHmmss");
                orderTicket.TotalPrice = seatList.Length * (movie != null ? movie.Price : 80000);
                var pointPlus = (int)(orderTicket.TotalPrice);

                if (bookTicketRequest.isUsePoint && user.PointsBalance > 0)
                {
                    var userPointsBalance = user.PointsBalance * 1000;
                    if(userPointsBalance > orderTicket.TotalPrice)
                    {
                        user.PointsBalance -= (int)(orderTicket.TotalPrice / 1000);
                        orderTicket.TotalPrice = 0;
                    }
                    else
                    {
                        orderTicket.TotalPrice -= (int)(user.PointsBalance * 1000);
                        user.PointsBalance = 0;
                    }
                }
                user.PointsBalance += pointPlus ;
