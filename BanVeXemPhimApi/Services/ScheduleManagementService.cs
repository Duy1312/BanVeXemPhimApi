...
namespace BanVeXemPhimApi.Services
{
    public class ScheduleManagementService
    {
        ...
        /// <returns></returns>
        public object GetSchedules(int limit, int page, int? cinemaId)
        {
            try
            {
                ...
                {
                    query = query.Where(row => row.CinemaId == cinemaId);
                }
        ...
        public object Store(ScheduleStoreRequest request)
        {
            try
            {
                ...
                {
                    throw new Exception("Phim chưa đến ngày khởi chiếu!");
                }
        ...
        public bool Delete(int scheduleId)
        {
            try
            {
                ...
                if(!string.IsNullOrEmpty(scheduleDelete.SeatHaveBeenBookedList.Trim()))
                {
                    throw new ValidateError("Lịch chiếu đã được đặt, không thể xoá");
                }
